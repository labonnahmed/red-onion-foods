...
import dinnerItems from '../HomeData/dinner.json';

const Dinner = ({ item }) => {
    return (
        <Link to='/' className='grid md:grid-cols-3 max-md:grid-cols-2 gap-4 mx-auto w-10/12 text-center mt-4'>
            {
                dinnerItems.map(item =>
                    <Link to={`/dinner/${item.title}`}>
